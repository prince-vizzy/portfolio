    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-[#217522] selection:text-white relative font-sans antialiased">
      ...
      <section id="work" className="py-40 px-12 md:px-24 md:pl-80 relative z-10">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-14 items-start">
          ...
          {/* Darts app card - opens the scorer */}
          <div
            className="group cursor-pointer max-w-2xl w-full"
            onClick={() => setShowDartsApp(true)}
            role="button"
            tabIndex={0}
            onKeyDown={e => e.key === 'Enter' && setShowDartsApp(true)}
      ...
      {showTitanicDashboard && (
        ...
        >
          {/* Fixed X button â€” always visible while scrolling */}
          <button
          ...
          {/* Dashboard content â€” slides up on enter */}
          <div
            style={{
              maxWidth: '1400px',
              margin: '0 auto',
      ...
      {showDartsApp && (
        ...
        >
          <button
            onClick={() => setShowDartsApp(false)}
            onMouseEnter={() => setCloseXHovered(true)}
            onMouseLeave={() => setCloseXHovered(false)}
            aria-label="Close darts app"
            style={{
              position: 'fixed',
              top: '20px',
              right: '24px',
